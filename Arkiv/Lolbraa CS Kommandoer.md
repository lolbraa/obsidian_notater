...
[Earnings](https://github.com/ReneSmeekes/storj_earnings)
````
cd /mnt/user/storj
python3 earnings.py
````

Graceful exit ????????? https://support.storj.io/hc/en-us/articles/360039432051-Graceful-Exit-Guide
````
docker exec -it storagenode /app/storagenode exit-satellite --config-dir /app/config --identity-dir /app/identity
````

Check graceful exit status:
````
docker exec -it storagenode-v3 /app/storagenode exit-status --config-dir /app/config --identity-dir /app/identity
````

[Sjekke åpne porter](https://storjnet.info/ping_my_node) 
